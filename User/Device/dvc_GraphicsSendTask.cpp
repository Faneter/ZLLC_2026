 ...
 * @param x_len, y_len 圆弧在X轴和Y轴方向上的半径。半径相等时绘制的是圆弧，不相等时则绘制椭圆弧。
 * @param line_width 圆弧线宽
 * @param color 圆弧颜色
 * @param name
 * @return graphic_data_struct_t*
 */
graphic_data_struct_t *Arc_Draw(uint8_t layer, int Op_Type, uint16_t startx, uint16_t starty, uint16_t start_angle, uint16_t end_angle, uint32_t x_len, uint32_t y_len, uint16_t line_width, int color, const uint8_t name[])
{
    static graphic_data_struct_t drawing;  // 定义为静态变量，避免函数调用时重复分配该变量内存
    memcpy(drawing.graphic_name, name, 3); // 图形名称，3位
    drawing.layer        = layer;
